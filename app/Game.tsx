...
export default function Game() {
  ...
  const finishCelebration = useCallback(() => {
    ...
    setCompletedOrders((value) => value.includes(finishedIndex) ? value : [...value, finishedIndex]);
  ...
  }, [activeOrder, celebrating, order.reward, order.title, orderIndex, orderReady]);
  ...
  const annaVisual = useMemo(() => {
    ...
    if (boredom >= 55) return { id: "bored", video: assetPath("assets/videos/anna-bored.mp4"), status: "Скучает", icon: "…", alt: "Анна скучает у швейной машинки" };
  ...
  const gameNav = <nav className="app-nav" aria-label="Разделы игры"><button type="button" className={screen === "home" ? "nav-active" : ""} disabled={celebrating} onClick={() => setScreen("home")}><span>⌂</span>Ателье</button><button type="button" className={screen === "orders" ? "nav-active" : ""} disabled={celebrating} onClick={() => setScreen("orders")}><span>▤</span>Заказы<b>{activeOrder ? 1 : hasAvailableOrder ? "!" : "✓"}</b></button>{activeOrder && <button type="button" className={screen === "match3" ? "nav-active" : ""} disabled={celebrating || orderReady} onClick={() => setScreen("match3")}><span>✂</span>{celebrating ? "Радуется" : orderReady ? "Готово" : "Шить"}</button>}{drawingUnlocked && <button type="button" className={`draw-nav${screen === "drawing" ? " nav-active" : ""}`} disabled={celebrating} onClick={() => setScreen("drawing")}><span>✎</span>Рисовать</button>}</nav>;

  const header = <><header className={screen === "match3" ? "topbar topbar-match-hidden" : "topbar"}><div className="brand-lockup"><div className="brand-mark" aria-hidden="true">А</div><div><p className="eyebrow">уютная история мастерской</p><h1>Ателье Анны</h1></div></div>{playerStats}</header>{gameNav}</>;
