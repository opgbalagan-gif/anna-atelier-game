...
export default function Game() {
  ...
  function finishCascade(points: number, collected: number[], combo: number) {
    ...
    setOrderProgress(newProgress); setOrderReady(newProgress >= order.goal);
    setBoredom((value) => Math.max(0, value - Math.min(14, 4 + combo * 2))); setBusy(false);
    setToast(newProgress >= order.goal ? "Все материалы собраны — нажмите «Заказ готов»" : combo > 1 ? `Каскад ×${combo}! +${points} очков` : `Отличный шов! +${points} очков`);
  }
  ...
  function careForAnna(kind: "food" | "rest" | "sew") {
    if (kind === "sew") {
      if (activeOrder) setScreen("match3"); else if (hasAvailableOrder) setShowOrderModal(true); else setToast("Все заказы на сегодня выполнены");
      return;
    }
  ...
  function completeOrder() {
    ...
    setOrderIndex((value) => value + 1); setBoredom((value) => Math.max(0, value - 28)); setCelebrating(true); setScreen("home");
    setToast(`Заказ «${finishedTitle}» готов! +${reward} монет`); window.setTimeout(() => setCelebrating(false), 5600);
  }
